...
class TestDBApi(unittest.TestCase):
    ...
    def test_api_random_salt_value(self):
        ...
        if host_web_app == "local":
            start_url = f"{ConfigReader.readWebApplicationProtocol()}{get_local_ip.get_ip()}:{port_web_app}"
        else:
            start_url = f"{ConfigReader.readWebApplicationProtocol()}{host_web_app}:{port_web_app}"
